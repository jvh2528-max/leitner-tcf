...
function putCard(card){
  return new Promise((resolve,reject)=>{
    const r=tx(STORE,'readwrite').put(card); r.onsuccess=()=>resolve(card); r.onerror=()=>reject(r.error);
  });
}
...
async function refresh(){
  ...
  for(let b=1;b<=5;b++){
    ...
    el.className=`box box${b}`; el.dataset.box=b;
    el.innerHTML=`<div><div class="boxName">خانه</div><div class="boxNum">${b}</div></div>
      <div><div class="boxName">${names[b]}</div><div class="boxCount">${list.length}</div><div class="boxDue">${d} آماده مرور · هر ${intervals[b]} روز</div></div>`;
...
async function openCards(boxFilter=null){
  ...
  list.sort((a,b)=>a.box-b.box||a.front.localeCompare(b.front)).forEach(c=>{
    ...
    div.innerHTML=`<div class="meta">خانه ${c.box} · مرور بعدی: ${new Date(c.nextReview).toLocaleDateString('fa-IR')}</div>
      <div class="word">${escapeHtml(c.front)}</div>
      <div>${escapeHtml(c.back)}</div>
      <div class="listActions">
        <button class="smallBtn" data-review="${c.id}">مرور همین کارت</button>
        <button class="smallBtn" data-delete="${c.id}">حذف</button>
      </div>`;
  ...
  wrap.querySelectorAll('[data-review]').forEach(btn=>btn.onclick=async()=>{
    ...
    reviewQueue=[c];reviewIndex=0;document.getElementById('cardsDialog').close();document.getElementById('reviewDialog').showModal();renderReview();
  });
  wrap.querySelectorAll('[data-delete]').forEach(btn=>btn.onclick=async()=>{
    if(confirm('این کارت حذف شود؟')){await deleteCard(btn.dataset.delete); openCards(boxFilter); refresh();}
  });
...
document.getElementById('settingsBtn').onclick=()=>alert('در نسخه بعد تنظیم زمان‌بندی، دارک‌مود و تنظیمات پیشرفته اضافه می‌کنیم.');
