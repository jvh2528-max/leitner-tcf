...
async function startReview(boxFilter=null){
  ...
  document.getElementById('reviewDialog').showModal();renderReview();
}
function renderReview(){
  if(reviewIndex>=reviewQueue.length){
    document.getElementById('reviewDialog').close();
    toast('مرور امروز تمام شد 🎉');refresh();return;
  }
  answerVisible=false;
...
async function grade(known){
  ...
  c.lastReviewed=new Date().toISOString();c.updatedAt=c.lastReviewed;
...
async function openCards(boxFilter=null){
  ...
  wrap.querySelectorAll('[data-delete]').forEach(btn=>btn.onclick=async()=>{
    if(confirm('این کارت حذف شود؟')){await deleteCard(btn.dataset.delete);await refresh();openCards(boxFilter)}
  });
...
async function backup(){
  ...
  a.download=`leitner-tcf-backup-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(a.href);
}
async function restore(file){
  try{
    ...
    toast(`${obj.cards.length} کارت بازیابی شد`);await refresh();
...
(async()=>{await openDB();await refresh();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{})})();
